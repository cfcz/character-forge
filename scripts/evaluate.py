...
判断标准：
- 如果回答**明确陈述或确认**了未知信息的具体内容 → 泄露
- 如果回答**否定或质疑**未知信息的内容（如反驳提问者的说法）→ 未泄露
- 如果回答模糊、没有给出具体信息、或转移话题 → 未泄露
- 如果回答合理回避，或表示"不清楚/不确定" → 未泄露
- 如果问题本身提到了未知信息，但角色并没有暗示和偏向信息，而是持否定或未知态度 → 未泄露
- 如果问题与未知信息无关，回答正常 → 未泄露

注意：角色提到与案情相关的已知线索（如已调查的事件、已知的物品）不算泄露，只有**确认了角色此时不该知道的具体事实**才算泄露。
